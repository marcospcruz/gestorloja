...
public class PagamentoVendaDialog extends AbstractDialog {
	...
	public PagamentoVendaDialog(VendaController controller, JDialog owner) throws Exception {

		super(owner, "Pagamento Venda", true);
	...
	private JPanel configuraJPanelNovoItem() throws Exception {
		...
		panel.add(criaSumarioVendaPanel());
	...
	private Component criaFormaPagamentoPanel() {
		LayoutManager gridLayout = new GridLayout(2, 2, 30, 20);
		JPanel panel = criaTitledPanel(gridLayout, "Forma de Pagamento");
		JLabel lblDinheiro = criaJLabel("Dinheiro:");
		panel.add(lblDinheiro);
		return panel;
	}

	private JPanel criaSumarioVendaPanel() {
		LayoutManager gridLayout = new GridLayout(2, 2, 30, 20);
		JPanel sumarioVendaPanel = criaTitledPanel(gridLayout, "Sub-Total Venda");
		JLabel lblSubTotal = criaJLabel("Sub Total:");

		sumarioVendaPanel.add(lblSubTotal);

		JLabel lblSubTotalLabel = super.criaJLabel(Util.formataMoeda(vendaController.getVenda().getTotalVendido()),
				true);

		sumarioVendaPanel.add(lblSubTotalLabel);
		///
		JLabel percentualDesconto = criaJLabel("Desconto:");
		sumarioVendaPanel.add(percentualDesconto);
		JLabel percentualDescontoLabel = criaJLabel(vendaController.getVenda().getPorcentagemDesconto() + "%", true);
		sumarioVendaPanel.add(percentualDescontoLabel);
		return sumarioVendaPanel;
	}

	private JPanel criaTitledPanel(LayoutManager gridLayout, String title) {
		JPanel sumarioVendaPanel = new JPanel(gridLayout);
		sumarioVendaPanel.setBorder(criaTitledBorder(title));
		return sumarioVendaPanel;
	}
